        ...
        {cart.length > 0 && (
          <div className="drawer-foot">
            ...
            <p style={{ fontSize: 11, color: 'var(--mute)', textAlign: 'center', marginTop: 14, fontWeight: 600 }}>
              🔒 Secured by Stripe · Sales tax included
            </p>
          </div>
        )}
      </aside>
        ...
        <div className="checkout">
              ...
              <div className="field-row cols-3" style={{ marginBottom: 16 }}>
                {['card', 'apple', 'paypal'].map(m => (
                  <button key={m} className={`option ${paymentMethod === m ? 'active' : ''}`} onClick={() => setPaymentMethod(m)} style={{ textAlign: 'center', padding: 18 }}>
                    <div className="option-name">{m === 'apple' ? 'Apple Pay' : m === 'paypal' ? 'PayPal' : 'Card'}</div>
                    <div className="option-meta">{m === 'card' ? 'Visa, MC, AmEx' : m === 'apple' ? 'Touch / Face ID' : 'Account'}</div>
                  </button>
                ))}
              </div>
              {paymentMethod === 'card' && (
          ...
          <aside className="checkout-summary">
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div className="summary-line"><span>Sales tax</span><span style={{ fontWeight: 700 }}>${tax.toFixed(2)}</span></div>
              <div className="summary-line total"><span>Total</span><span>${total.toFixed(2)}</span></div>
            </div>
          </aside>
        </div>
